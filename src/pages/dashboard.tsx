...
import { useRoqPlatformUserProfiles, useAppointmentCount, usePrescriptionCount, useScheduleCount } from 'lib/roq';

import { PrescriptionListPage as ShowCaseList } from 'pages/prescriptions';
...
function DashboardPage() {
  ...
  const prescriptionStat = usePrescriptionCount();

  const scheduleStat = useScheduleCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Prescriptions',
      result: prescriptionStat?.data,
      isLoading: prescriptionStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Schedules',
      result: scheduleStat?.data,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`prescriptions`} passHref legacyBehavior>
                  <Button
