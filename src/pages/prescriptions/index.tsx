...
import { useRoqClient, usePrescriptionFindManyWithCount } from 'lib/roq';
...
export function PrescriptionListPage(props: PrescriptionListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['user_prescription_doctor_idTouser', 'user_prescription_patient_idTouser'],
          searchTerm: params.searchTerm,
          searchTermKeys: ['medicine_name.contains', 'dosage.contains', 'frequency.contains', 'duration.contains'],
          ...params.filters,
        },
        'prescription',
      ),
  ...
  const { data, error, isLoading, mutate } = usePrescriptionFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.prescription.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    { id: 'medicine_name', header: 'Medicine Name', accessorKey: 'medicine_name' },
    ...
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_prescription_doctor_idTouser',
          header: 'User Prescription Doctor Id Touser',
          accessorKey: 'user_prescription_doctor_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_prescription_doctor_idTouser?.id}`}
            >
              {record.user_prescription_doctor_idTouser?.email}
            </Link>
          ),
        }
      : null,
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_prescription_patient_idTouser',
          header: 'User Prescription Patient Id Touser',
          accessorKey: 'user_prescription_patient_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_prescription_patient_idTouser?.id}`}
            >
              {record.user_prescription_patient_idTouser?.email}
            </Link>
          ),
        }
      : null,
